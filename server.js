...
function calcularEscenarios(standings, scheduledMatches) {
  ...
  // Detectar grupos con partidos pendientes por partidos jugados
  // Si algún equipo jugó menos de 3, el grupo tiene partido pendiente
  const gruposPendientes = new Set();
  for (const grp in grupos) {
    const maxJugados = Math.max(...grupos[grp].map(t => t.played || (t.w + t.d + t.l)));
    if (maxJugados < 3) {
